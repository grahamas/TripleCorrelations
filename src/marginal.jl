...
function time_tricorr_zeropad!(time_contribution_offset::OffsetMatrix, src::AbstractArray{T}, lag_extents) where T
    ...
    time_contribution ./ calculate_scaling_factor(src, (lag_extents))
    return
end

function time_tricorr_zeropad(src, lag_extents)
    time_lag_range = -(time_max_lag):time_max_lag
    time_contributions = OffsetArray{Float64}(undef, time_lag_range, time_lag_range)
    time_tricorr_zeropad!(time_contributions, src, lag_extents)
...
function space_time_tricorr_zeropad!(space_time_contribution_offset::OffsetMatrix, src::AbstractArray{T}, lag_extents) where T
    ...
    space_time_contribution ./= calculate_scaling_factor(src, (lag_extents))
    return
end

function space_time_tricorr_zeropad(src, lag_extents)
    time_lag_range = -(time_max_lag):time_max_lag
    space_lag_range = -(space_max_lag):space_max_lag
    space_time_contributions = OffsetArray{Float64}(undef, space_lag_range, time_lag_range)
    space_time_tricorr_zeropad!(space_time_contributions, src, lag_extents)
...
function space_tricorr_zeropad!(space_contribution_offset::OffsetMatrix, src::AbstractArray{T}, lag_extents) where T
    ...
    space_contribution ./= calculate_scaling_factor(src, (lag_extents))
    return
end

function space_tricorr_zeropad(src, lag_extents)
    space_lag_range = -(space_max_lag):space_max_lag
    space_contributions = OffsetArray{Float64}(undef, space_lag_range, space_lag_range)
    space_tricorr_zeropad!(space_contributions, src, lag_extents)
...
function marginal_tricorr_zeropad!(marginal_contributions_offset::NamedTuple, src::AbstractArray{T}, lag_extents) where T
    ...
    scaling = calculate_scaling_factor(src, (lag_extents))
...
function marginal_tricorr_zeropad(src, lag_extents)
    time_lag_range = -(time_max_lag):time_max_lag
    space_lag_range = -(space_max_lag):space_max_lag
    space_time_contributions = OffsetArray{Float64}(undef, space_lag_range, time_lag_range)
    time_contributions = OffsetArray{Float64}(undef, time_lag_range, time_lag_range)
    ...
    marginal_tricorr_zeropad!(marginal_contributions, src, lag_extents)
